...
- **`scripts/include-layout.js`** — refactored: extracted `injectSidebar()` (shared `.pub-layout`/`.pub-sidebar` wrapping), `markActiveByFilename()`, and `initScrollSpy()`. **Publications** inject on `/publications/` (one series expands). **Projects** inject on `projects.html` **and every `/projects/` detail page** — both groups kept expanded (full list always visible) and the current project highlighted by filename; section-header links are excluded from matching so the grid page doesn't false-highlight both headers. **Resume** uses an **IntersectionObserver** scroll-spy (`rootMargin: -120px 0 -65% 0`) observing leaf links only.
- **Collapsible first level (follow-up):** `wireCollapsibles()` in `include-layout.js` wraps every group header that has sub-items in a `.pub-nav-series-row` and prepends a `.pub-nav-toggle` chevron **button**. The header still navigates; the chevron expands/collapses the group (toggling `.active-group`, with `aria-expanded` synced). Applies to **publications + projects** sidebars (resume groups are childless, so unaffected). Styled in `style.css` (`.pub-nav-series-row` / `.pub-nav-toggle`); the old `::before` arrow is hidden inside a row.
