...
- **Collapsible first level (follow-up):** `wireCollapsibles(aside, headerToggles)` in `include-layout.js` wraps every group header that has sub-items in a `.pub-nav-series-row` and prepends a `.pub-nav-toggle` chevron **button** (toggles `.active-group`, `aria-expanded` synced). **Publications:** chevron toggles; the header link still navigates to the series landing (which then shows the group expanded). **Projects** (`headerToggles=true`): the whole header row toggles the group **in place** (`preventDefault`) — projects headers point at the grid sections, not a per-group landing, so a header click expands rather than navigates. Resume groups are childless, so unaffected. Styled in `style.css` (`.pub-nav-series-row` / `.pub-nav-toggle`); the old `::before` arrow is hidden inside a row.
